...
AddEventHandler("chatMessage", function(s, n, m)
  ...
  if command[1] == "/checkbalance" then
    TriggerClientEvent("chatMessage", s, "BANK", {0, 200, 0}, "Seu saldo atual é: ^3$" .. bankbalance)
    TriggerClientEvent("banking:updateBalance", source, bankbalance)
    CancelEvent()
  end

  if command[1] == "/depositar" then
    local amount = tonumber(command[2])
    TriggerClientEvent('bank:deposit', s, amount)
    CancelEvent()
  end

  if command[1] == "/retirar" then
    local amount = tonumber(command[2])
    TriggerClientEvent('bank:withdraw', s, amount)
    CancelEvent()
  end

  if command[1] == "/transferir" then
    if(command[2] ~= nil and tonumber(command[3]) > 0) then
      ...
      TriggerClientEvent('chatMessage', s, "BANK", {0, 200, 0}, "^1Use /transfererir [id] [amount]^0")
    end
    CancelEvent()
  end
end)
...
AddEventHandler('bank:deposit', function(amount)
  if not amount then return end
    ...
    if(string.len(rounded) >= 9) then
      vRPclient.notify(user_id,"~r~Entrada muito alta.")
    else
      local bankbalance = vRP.getBankMoney(user_id)
      if(rounded <= wallet) then
        ...
        vRPclient.notify(user_id,"~r~Não tem dinhero!")
      end
    end
end)
  ...
  if not amount then return end
    ...
    if(string.len(rounded) >= 9) then
      ...
      if(tonumber(rounded) <= tonumber(bankbalance)) then
        ...
        vRPclient.notify(user_id, "~r~Não tem dinheiro na conta!")
      end
    end
end)
...
AddEventHandler('bank:transfer', function(fromPlayer, toPlayer, amount)
  ...
  if tonumber(fPlayer) == tonumber(toPlayer) then
    TriggerClientEvent('chatMessage', fPlayer, "", {0, 255, 0}, "^1Não pode se transferir")
    CancelEvent()
  else
    local rounded = round(tonumber(amount), 0)
    if(string.len(rounded) >= 9) then
      TriggerClientEvent('chatMessage', fPlayer, "", {0, 255, 0}, "^1Entrada muito alta!")
      CancelEvent()
    else
      local bankbalance = vRP.getBankMoney(user_id)
      if(tonumber(rounded) <= tonumber(bankbalance)) then
        ...
        TriggerClientEvent('chatMessage', fPlayer, "", {0, 255, 0}, "^0Transferido: ^1$"..math.floor(rounded))
        TriggerClientEvent('chatMessage', fPlayer, "", {0, 255, 0}, "^0Novo saldo: ^4$" .. math.floor((bankbalance - rounded)))
        ...
        TriggerClientEvent('chatMessage', toPlayer, "", {0, 255, 0}, "^0Recebido: ^2$"..math.floor(rounded))
        TriggerClientEvent('chatMessage', toPlayer, "", {0, 255, 0}, "^0Novo balance: ^4$" .. math.floor((bankbalance2 + rounded)))
        CancelEvent()
      else
        TriggerClientEvent('chatMessage', fPlayer, "", {0, 255, 0}, "^1Nao tem dinheiro na conta!")
      end
    end    
  end
end)
