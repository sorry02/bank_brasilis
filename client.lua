...
local banks = {
  ...
  {name="Banco Safra", id=108, x=-1212.980, y=-330.841, z=37.787},
  {name="Banco Caixa Economica", id=108, x=-2962.582, y=482.627, z=15.703},
  {name="Banco Itau", id=108, x=-112.202, y=6469.295, z=31.626},
  {name="Banco do Bradesco", id=108, x=314.187, y=-278.621, z=54.170},
  {name="Banco Santander", id=108, x=-351.534, y=-49.529, z=49.042},
  {name="Casa Da Moeda Brasileira", id=108, x=241.727, y=220.706, z=106.286},
  {name="Banco Original", id=108, x=1176.86865234375, y=2711.91357421875, z=38.097785949707},
}
    ...
    while true do
      ...
      if(IsNearBank() or IsNearATM()) then
        if (atBank == false) then
          TriggerEvent('chatMessage', "", {255, 255, 255}, "Press ^5E^0 para acessar o banco");
        end
        atBank = true
        if IsControlJustPressed(1, 51)  then -- IF INPUT_PICKUP Is pressed
          if (IsInVehicle()) then
            TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Você não pode usar o banco do veiculo!");
          else
            if bankOpen then
              closeGui()
              bankOpen = false
...
RegisterNUICallback('quickCash', function(data, cb)
  TriggerEvent('bank:withdraw', 1500)
  cb('ok')
end)
...
AddEventHandler('bank:deposit', function(amount)
  if(IsNearBank() == true or depositAtATM == true and IsNearATM() == true or depositAnywhere == true ) then
    if (IsInVehicle()) then
      TriggerEvent('chatMessage', "", {0, 255, 0}, "^1Você não pode usar ATM no veiculo");
    else
      TriggerServerEvent("bank:deposit", tonumber(amount))
      TriggerEvent('chatMessage', "BANK", {0, 200, 0}, "^0Você depositou ^5$"..amount);
    end
  else
    TriggerEvent('chatMessage', "", {0, 255, 0}, "^1Você só pode depositar em um banco!");
  end
end)
...
AddEventHandler('bank:withdraw', function(amount)
  if(IsNearATM() == true or IsNearBank() == true or withdrawAnywhere == true) then
    if (IsInVehicle()) then
      TriggerEvent('chatMessage', "", {0, 255, 0}, "^1Você não pode usar o banco estando dentro do veiculo");
    else
      TriggerServerEvent("bank:withdraw", tonumber(amount))
      TriggerEvent('chatMessage', "BANK", {0, 200, 0}, "^0Você sacou ^5$"..amount);
    end
  else
    TriggerEvent('chatMessage', "", {0, 255, 0}, "^1Esse não é um banco nem um caixa eletrônico!");
  end
end)
...
AddEventHandler('bank:givecash', function(toPlayer, amount)
  if(IsNearPlayer(toPlayer) == true or giveCashAnywhere == true) then
    ...
    if (playing ~= false) then
      ...
      TriggerEvent('chatMessage', "", {0, 255, 0}, "^1Essa pessoa não está na cidade!");
    end
  else
    TriggerEvent('chatMessage', "", {0, 255, 0}, "^1Você não está perto de nenhuma pessoa!");
  end
end)
